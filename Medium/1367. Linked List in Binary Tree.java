class Solution {
    ...
    boolean dfs(ListNode head, ListNode cur, TreeNode root) {
        ...
        if (cur == null)
            return true;
        // If we reach the end of a path in the tree without fully matching the list,
        // return false
        if (root == null)
            return false;
        ...
        if (cur.val == root.val)
            cur = cur.next;
        // If no match, but the tree node matches the head of the linked list, start a
        // new match
        else if (head.val == root.val)
            head = head.next;
        // Otherwise, reset `cur` to `head` to attempt matching the linked list from
        // scratch
        else
            cur = head;
