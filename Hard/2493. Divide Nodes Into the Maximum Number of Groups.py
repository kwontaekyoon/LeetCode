...
class Solution:
    def magnificentSets(self, n: int, edges: List[List[int]]):
        # Helper function to check bipartiteness and collect component nodes
        def is_bipartite(node: int, c: int, component: List[int]) -> bool:
            color[node] = c
        ...
        def max_groups_in_component(component: List[int]) -> int:
            max_depth = 0
